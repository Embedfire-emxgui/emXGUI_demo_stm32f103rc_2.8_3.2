...
{ 
   switch(msg){
      ...
      case WM_PAINT:
      {
        ...
        DrawText(hdc, L"V1", -1, &rc, DT_VCENTER|DT_RIGHT);//绘制文字(居中对齐方式) 
        HLine(hdc, rc.x, rc.y+rc.h, rc.x+310);

        OffsetRect(&rc,0,rc.h);
        DrawText(hdc, L"开发板型号", -1, &rc, DT_VCENTER|DT_LEFT);//绘制文字(居中对齐方式)
        DrawText(hdc, L"野火-MINI", -1, &rc, DT_VCENTER|DT_RIGHT);//绘制文字(居中对齐方式) 
        HLine(hdc, rc.x, rc.y+rc.h, rc.x+310);

        OffsetRect(&rc,0,rc.h);
        DrawText(hdc, L"储存空间", -1, &rc, DT_VCENTER|DT_LEFT);//绘制文字(居中对齐方式)
        DrawText(hdc, L"256KB", -1, &rc, DT_VCENTER|DT_RIGHT);//绘制文字(居中对齐方式) 
        HLine(hdc, rc.x, rc.y+rc.h, rc.x+310);

        OffsetRect(&rc,0,rc.h);
        DrawText(hdc, L"运行内存", -1, &rc, DT_VCENTER|DT_LEFT);//绘制文字(居中对齐方式)
        DrawText(hdc, L"48KB", -1, &rc, DT_VCENTER|DT_RIGHT);//绘制文字(居中对齐方式) 
        HLine(hdc, rc.x, rc.y+rc.h, rc.x+310);
        ...
        DrawText(hdc, L"STM32F103RCT6", -1, &rc, DT_VCENTER|DT_RIGHT);//绘制文字(居中对齐方式) 
        HLine(hdc, rc.x, rc.y+rc.h, rc.x+310);
...
HWND hwnd_d;
extern HWND GUI_App_HWND;//GUI应用APP的滑动主窗口句柄
      ...
      case WM_NOTIFY:
      {
        ...
        if(code == BN_CLICKED)
        {
          ...
          else if (id == ID_SETTINGS_THEME)
          {
            ...
            {
              Theme_Flag = 1;
              SendMessage(GetDlgItem(hwnd_home, 0x1000), MSG_SET_BGCOLOR, 2, NULL);   		// 0x1000 是home 的列表ID
              SendMessage(GetDlgItem(GUI_App_HWND, 0x1000), MSG_SET_BGCOLOR, 2, NULL);    // 0x1000 是home 的列表ID,GUI应用APP的滑动主窗口句柄，改变主题时两个滑动窗口的主题标志位都要改变
              SetWindowText(wnd, L"2");
            }
            else
            {
              SendMessage(GetDlgItem(hwnd_home, 0x1000), MSG_SET_BGCOLOR, 1, NULL);       // 设置为 1 时 使用另一个颜色作为背景  0x1000 是home 的列表ID
              SendMessage(GetDlgItem(GUI_App_HWND, 0x1000), MSG_SET_BGCOLOR, 1, NULL);    // 0x1000 是home 的列表ID
              Theme_Flag = 0;
