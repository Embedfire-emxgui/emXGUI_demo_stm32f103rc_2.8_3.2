        ...
        case BN_CLICKED:
        {
          switch (id)
          {
            ...
            case eID_SMS_SEND:
            {
              ...
              wContent = (WCHAR *)GUI_VMEM_Alloc(128*sizeof(WCHAR));
              
              GetWindowText(GetDlgItem(hwnd, eID_SMS_NUMBER), wNumber, 99);
              GetWindowText(GetDlgItem(hwnd, eID_SMS_CONTENT), wContent, 128);
              x_wcstombs(cNumber, wNumber, 99);
              x_wcstombs(cContent, wContent, 128);
            ...
            case eID_SMS_LIST:
            {
              ...
              WCHAR *wContent;
              cNumber  = (char  *)GUI_VMEM_Alloc(100);
							/* ↓注①要想显示更多内容,可以调整这里的缓冲区大小,但是要注意内存够不够用 */
              cContent = (char  *)GUI_VMEM_Alloc(512);
              wNumber  = (WCHAR *)GUI_VMEM_Alloc(100*sizeof(WCHAR));
							/* ↓注①这里的大小要同步 */
              wContent = (WCHAR *)GUI_VMEM_Alloc(512*sizeof(WCHAR));
              
              /* 读短信内容 */
              if (readmessage(ListData & 0x7FFF, cNumber, cContent))
              {
                hexuni2gbk((char *)cNumber, (char *)&cNumber[50]);	
                hexuni2gbk((char *)cContent, (char *)&cContent[256]);
                
//                GUI_DEBUG("cNumber:(%s)", (char *)&cNumber[100]);
//                GUI_DEBUG("cContent:(%s)", (char *)&cContent[2000]);
                
                x_mbstowcs(wNumber, (char *)&cNumber[50], 50);	    // 将Ansi字符转换成GUI的unicode字符.
                x_mbstowcs(wContent, (char *)&cContent[256], 256);	// 将Ansi字符转换成GUI的unicode字符.
