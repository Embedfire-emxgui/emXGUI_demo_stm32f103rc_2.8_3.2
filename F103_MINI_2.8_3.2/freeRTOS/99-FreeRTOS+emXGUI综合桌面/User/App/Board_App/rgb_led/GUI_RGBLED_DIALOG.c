  ...
  RECT rc;
	switch (msg)
	{
      case WM_CREATE: 
      {
         ...
         CreateWindow(TEXTBOX, L"流水灯控制", WS_TRANSPARENT|WS_VISIBLE, 
         ...
         /*创建滑动条--G*/
         CreateWindow(SCROLLBAR, L"SCROLLBAR_G", WS_OWNERDRAW |WS_TRANSPARENT|WS_VISIBLE, 
...
//                        hwnd, ID_RGB_SELECT_R | (1 << 16), NULL, NULL); 
         ...
         /* 单选按钮 */
         CreateWindow(BUTTON, L"流水灯", WS_OWNERDRAW | WS_VISIBLE | BS_RADIOBOX,
...
//                        hwnd, ID_RGB_SELECT_B | (1 << 16), NULL, NULL); 
        ...
        SendMessage(GetDlgItem(hwnd, ID_RGB_SELECT_G), BM_SETSTATE, BST_CHECKED, 1);    // 默认选中第一个
         ...
         SetTimer(hwnd,0x05,255-RGBLED_DIALOG.col_G,TMR_START,NULL);
				 SetTimer(hwnd,0x06,20,TMR_START,NULL);
				 LED1_ON;
				 LED2_OFF;
         ...
         break;
			...
			case WM_TIMER:
			{
				...
				if(timer_id == 0x05)
				{
					ResetTimer(hwnd,0x05,255-leddlg_S.col_G,TMR_START,NULL);
				}
			}break;
